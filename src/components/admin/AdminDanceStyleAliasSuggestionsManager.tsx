...
function isItem(v: unknown): v is Item {
  ...
  return (
    typeof v.id === "string" &&
    typeof v.aliasName === "string" &&
    typeof v.status === "string" &&
    typeof v.createdAt === "string" &&
    isRecord(v.style) &&
    typeof (v.style as Record<string, unknown>).name === "string" &&
    isRecord(v.createdBy) &&
    typeof (v.createdBy as Record<string, unknown>).email === "string"
  );
}

export default function AdminDanceStyleAliasSuggestionsManager() {
  ...
  const load = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await fetch("/api/admin/dance-style-alias-suggestions", { cache: "no-store" });
  ...
  const decide = async (id: string, action: "APPROVE" | "REJECT") => {
    setBusyId(id);
    try {
      const res = await fetch(`/api/admin/dance-style-alias-suggestions/${encodeURIComponent(id)}`, {
        method: "PATCH",
      ...
      if (!res.ok) {
        showToast((data as { message?: string })?.message || "Aktion fehlgeschlagen", "error");
        return;
      }
      await load();
  ...
  if (isLoading) return <div className="p-8 text-center text-gray-900 dark:text-gray-100">Laden...</div>;
  ...
  return (
    <div className="bg-white dark:bg-gray-800 shadow sm:rounded-lg overflow-hidden border border-transparent dark:border-gray-700">
      <div className="px-4 py-5 sm:px-6 border-b border-gray-200 dark:border-gray-700">
        <h2 className="text-lg font-medium text-gray-900 dark:text-white">Alias-Vorschläge</h2>
            ...
            <li key={x.id} className="px-4 py-4 sm:px-6">
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div className="min-w-0">
                  <div className="text-sm font-semibold text-gray-900 dark:text-gray-100 break-words">{x.aliasName}</div>
                  <div className="mt-1 text-sm text-gray-500 dark:text-gray-400 break-words">
                    → Zielstil: <span className="font-medium">{x.style.name}</span>
                  </div>

                  {x.sourceUrl ? (
                    <div className="mt-1 text-sm">
                      <a href={x.sourceUrl} target="_blank" rel="noreferrer" className="text-indigo-600 dark:text-indigo-300 hover:underline break-all">
                        {x.sourceUrl}
                      </a>
                    </div>
                  ) : null}

                  {x.comment ? <div className="mt-2 text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{x.comment}</div> : null}

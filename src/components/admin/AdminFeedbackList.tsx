...
export default function AdminFeedbackList({ initialItems }: { initialItems: FeedbackRow[] }) {
  ...
  const [isSavingNotifyEmail, setIsSavingNotifyEmail] = useState(false);
  ...
  const loadNotifyEmail = async () => {
    try {
      ...
      setNotifyEmail(typeof data?.FEEDBACK_NOTIFY_EMAIL === "string" ? data.FEEDBACK_NOTIFY_EMAIL : "");
    } catch (e) {
  ...
  const saveNotifyEmail = async () => {
    setIsSavingNotifyEmail(true);
    try {
      const res = await fetch("/api/admin/settings", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ FEEDBACK_NOTIFY_EMAIL: notifyEmail }),
      });
      const data = await res.json().catch(() => null);
    ...
    <div className="space-y-4">
      <div className="bg-white dark:bg-gray-800 shadow overflow-hidden sm:rounded-lg">
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <div className="text-sm font-semibold text-gray-900 dark:text-gray-100">E-Mail Benachrichtigung</div>
          <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">
            Wenn gesetzt, erhält diese Adresse eine E-Mail bei neuem Feedback (SMTP muss konfiguriert sein).
          </div>
        </div>
        <div className="px-6 py-4">
          <div className="flex flex-col sm:flex-row gap-3 sm:items-end">
            <div className="flex-1">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Empfänger</label>
              <input
                type="email"
                value={notifyEmail}
                onChange={(e) => setNotifyEmail(e.target.value)}
                placeholder="dev@example.com"
                className="mt-1 block w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-950 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
            </div>
            <button
              type="button"
              onClick={saveNotifyEmail}
              disabled={isSavingNotifyEmail}
            ...
            >
              {isSavingNotifyEmail ? "Speichere…" : "Speichern"}
            </button>
          </div>
        </div>
      </div>

  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden mt-8">
      <div className="bg-gray-50 px-6 py-4 border-b border-gray-200">
        <h3 className="font-bold text-gray-900">Anfragen & Teilnehmer verwalten</h3>
      </div>
        ...
        {pending.length > 0 && (
          <div>
            ...
            <div className="space-y-3">
              {pending.map(p => (
                <div key={p.id} className="flex flex-col sm:flex-row items-center justify-between p-4 bg-orange-50 rounded-lg border border-orange-100 gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                        <div className="font-bold text-gray-900">{p.group.name}</div>
                        {p.message && <div className="text-sm text-gray-600 italic">&quot;{p.message}&quot;</div>}
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {approved.map(p => (
                    <div key={p.id} className="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-lg">
                        <div className="flex items-center gap-3">
                            <div className="font-medium text-gray-900">{p.group.name}</div>
                        </div>
                        <button 
                            onClick={() => handleUpdateStatus(p.id, "REJECTED")}
                            disabled={isLoading === p.id}

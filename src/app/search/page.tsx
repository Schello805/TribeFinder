...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const [groups, events, dancers, styles] = q.length >= 3
    ? await Promise.all([
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8 px-4 py-8">
      <div>
        ...
        {q.length > 0 && q.length < 3 ? <p className="mt-2 text-sm text-[var(--muted)]">Die Suche startet automatisch ab drei Zeichen.</p> : null}
        {q.length >= 3 ? <p className="mt-2 text-sm text-[var(--muted)]">{total} Treffer für „{q}“</p> : null}
      </div>

      {q.length >= 3 && total === 0 ? <div className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-8 text-center text-[var(--muted)]">Keine passenden Ergebnisse gefunden.</div> : null}

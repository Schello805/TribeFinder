...
export async function POST(req: Request) {
  ...
  try {
    ...
    const filename = `branding-hero-logo-${crypto.randomUUID()}${ext}`;
    ...
    const oldSetting = await prisma.systemSetting.findUnique({
      where: { key: "BRANDING_HERO_LOGO_URL" },
    });

    await prisma.systemSetting.upsert({
      where: { key: "BRANDING_HERO_LOGO_URL" },
      update: { value: logoUrl },
      create: { key: "BRANDING_HERO_LOGO_URL", value: logoUrl },
    });
...
export async function DELETE() {
  ...
  try {
    const setting = await prisma.systemSetting.findUnique({ where: { key: "BRANDING_HERO_LOGO_URL" } });
    ...
    await prisma.systemSetting.delete({ where: { key: "BRANDING_HERO_LOGO_URL" } }).catch(() => undefined);
